...
export default function Select({ defaultData, sendData, dataSource, customStyle }){
  ...
  return (
    <Container style={customStyle} ref={ref}>
      <div onClick={() => setActive(!active)} className={active ? 'active': ''}>
        <p>
          {(dataList[dataSource].find(item => item.value === defaultData)) ? (dataList[dataSource].find(item => item.value === defaultData)).name : defaultData}
        </p>
        <ChevronRight />
      </div>
      {active && (
        <List>
          {dataList[dataSource].map((item, index) => 
            <li key={index} onClick={() => handleListClick(item.value)}>{item.name}</li>
          )}
        </List>
      )}
    </Container>
